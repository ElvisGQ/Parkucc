...
import com.example.parkucc.databinding.FragmentParkingSectionB2Binding;
...
public class ParkingSectionB2 extends Fragment {

    private FragmentParkingSectionB2Binding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragmentParkingSectionB2Binding.inflate(inflater, container, false);
        View root = binding.getRoot();

        availableSpaces = 0;

        // Navegación entre secciones
        ImageView flechaSeccionB2haciaB1 = binding.flechaSeccionB2haciaB1;
        flechaSeccionB2haciaB1.setOnClickListener(v ->
                Navigation.findNavController(v).navigate(R.id.action_parkingSectionB2_to_parkingSectionB1)
        );

        ImageView flechaSeccionB2haciaB3 = binding.flechaSeccionB2haciaB3;
        flechaSeccionB2haciaB3.setOnClickListener(v ->
                Navigation.findNavController(v).navigate(R.id.action_parkingSectionB2_to_parkingSectionB3)
        );

        // Inicializar botones y carros para los espacios 31 al 33 y 35 al 40
        buttons = new Button[]{
                binding.buttonCar31, binding.buttonCar32, binding.buttonCar33,
                binding.buttonCar35, binding.buttonCar36, binding.buttonCar37,
                binding.buttonCar38, binding.buttonCar39, binding.buttonCar40
        };

        cars = new ImageView[]{
                binding.car31, binding.car32, binding.car33,
                binding.car35, binding.car36, binding.car37, binding.car38,
                binding.car39, binding.car40
        };
                        ...
                        requireActivity().runOnUiThread(() -> {
                            for (int i = 0; i < jsonArray.length(); i++) {
                                try {
                                    ...
                                    if ((idEspacio >= 31 && idEspacio <= 33) || (idEspacio >= 35 && idEspacio <= 40)) {
                                        int index = (idEspacio >= 35) ? idEspacio - 32 : idEspacio - 31; // Ajustar índices después del espacio 34

                                        if (index >= 0 && index < cars.length) { // Verificar que el índice está dentro del rango del array
                    ...
                    try {
                        ...
                        requireActivity().runOnUiThread(() -> {
                            if (status == 200) {
                                ...
                                popupWindow.dismiss();
                                int index = Integer.parseInt(espacio) - 31;
